function [spiketimes]=ILIFnetwork(n,W)
...
current_type = 2; %select current type
                  %1 = constant applied current
                  %2 = current pulse
                  
switch(current_type)
    case 1 % constant applied current
        Iapp = 4*ones(n,1);
        %Iapp = 2*ones(n,1);
        if n>=10
        Iapp(10)=4;
    ...
    case 2 % pulse of applied current
        ton=20;
        toff=30;
        pulsei=6;

    otherwise 
        disp('not a valid current type')
end
